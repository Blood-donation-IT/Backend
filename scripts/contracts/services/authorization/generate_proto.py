...
def main():
    ...
    authorization_dir = os.path.join(PROTO_OUT, "authorization")
    os.makedirs(authorization_dir, exist_ok=True)
    open(os.path.join(PROTO_OUT, "__init__.py"), "a").close()
    open(os.path.join(authorization_dir, "__init__.py"), "a").close()
    ...
    proto_files = []
    authorization_proto_dir = os.path.join(PROTO_SRC, "authorization")
    if not os.path.isdir(authorization_proto_dir):
        print(f" Proto source directory not found: {authorization_proto_dir}")
        sys.exit(1)
    for filename in os.listdir(authorization_proto_dir):
        if filename.endswith(".proto"):
            proto_path = os.path.join("authorization", filename).replace("\\", "/")
    ...
    grpc_file = os.path.join(PROTO_OUT, "authorization", "authorization_pb2_grpc.py")
